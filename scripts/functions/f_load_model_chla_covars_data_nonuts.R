...
f_load_model_chla_covars_data <- function() {
  ...
  chla_nuts0 <- readr::read_csv(chla_nuts_file) %>%
    ...
    mutate(region = case_when(region == "main_above"~ "above",
                              region == "main_below" ~ "below",
                              region == "off_channel_below" ~ "cache",
                              TRUE ~ region)) %>%
    # select(-method)%>%
  ...
  # Rearrange data for regions----------------------------------------
    ...
    # chla_nuts <- left_join(chla_nuts0, nuts, by = "unique_id")
    ...
    pivot_longer(cols = c(Q_above, Q_yolo, Q_below, Q_cache,
      ...
      select(-station_flow, -agency_cd, -stage_SRV, -height_sac, -sac, -daymet_tmax_yolo,
             -daymet_tmax_below, -daymet_tmax_above, -daymet_tmax_cache,
