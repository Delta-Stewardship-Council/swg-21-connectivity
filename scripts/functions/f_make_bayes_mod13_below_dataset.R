...
f_make_bayes_mod13_below_dataset <- function() {
  ...
  # Plot chl by region and station
  chl_regions <- ggplot(chla_all, aes(x = date, y = chlorophyll, group = station_wq_chl, color = station_wq_chl, label = unique_id)) + facet_wrap(facets = "region", nrow = 2) + geom_point() +
    geom_text(aes(label=ifelse(chlorophyll>20,as.character(unique_id),'')),hjust = 1, size = 3)
  ggsave("bayes_models/mod13_below/fig_out/mod13_chl_regions.png", chl_regions)

  # For this model, we focus only on the "below" region because times sampled are very different by region
  chla_all <- chla_all %>% filter(region == "below") %>% mutate(station_id = as.numeric(as.factor(station_wq_chl)))
  chla_all$station_wq_chl <- as.factor(chla_all$station_wq_chl)
  covars_1 <- covars_1 %>% filter(region == "below")
  ...
  write_rds(chla_all, "bayes_models/mod13_below/mod_chla_data.rds")
  write_rds(covars_scaled, file = "bayes_models/mod13_below/mod_covariates_complete.rds")
