...
f_make_bayes_mod19inundposs_wtemponly_dataset <- function() {
  ...
  chla_above <- chla_all %>% filter(date > "1999-02-22", date < "2019-10-01") %>% filter(region == "above") %>%
   drop_na()

  covars_above <- mod_df %>% select(-chlorophyll,-latitude, -longitude, -source, -doy1998) %>%
    filter(region == "above") %>% filter(date > "1999-02-22")
  ...
  # Join
  covars_above <- left_join(days, covars_above)

  # Fill remaining NAs
  covars_checkNAs <- covars_above %>% filter(date > "1999-02-22") %>% filter(is.na(water_year))
  covars_datesNAs <- covars_checkNAs %>% select(date)

  covars_above_fillNAs <- mod_df %>% select(-chlorophyll, -latitude, -longitude, -source, -doy1998) %>%
    filter(region == "above") %>% filter(date > "1999-02-22")

  covars_NAs_filled <- left_join(covars_datesNAs, covars_above_fillNAs)
  covars_above <- covars_above %>% filter(date > "1999-02-22")
  covars_above_1 <- bind_rows(covars_NAs_filled, covars_above) %>% arrange(date) %>%
  ...
  covars_above_scaled <- covars_above_1 %>%
  ...
  covars_above_scaled$Q <- as.numeric(covars_above_scaled$Q)
  covars_above_scaled$Srad_mwk <- as.numeric(covars_above_scaled$Srad_mwk)
  covars_above_scaled$inund_days <- as.numeric(covars_above_scaled$inund_days)
  covars_above_scaled$Wtemp_mwk <- as.numeric(covars_above_scaled$Wtemp_mwk)

  # make list to save out
  model_df <- list("chla_above"=chla_above, "covars_above"=covars_above_scaled)

  # write out
  # if needed write to rds files:
  write_rds(chla_above, "bayes_models/mod19_above_inund_poss_sradonly/mod_chla_data.rds")
  write_rds(covars_above_scaled, file = "bayes_models/mod19_above_inund_poss_sradonly/mod_covariates_complete.rds")
