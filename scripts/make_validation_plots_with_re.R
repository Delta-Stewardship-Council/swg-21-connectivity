...
# Make plots ----------------------------------------------------------------
## Downstream -------------------------------------------------------------
resid_d = residuals(gamd6d)
res_d <- data.frame(fitted = fitted(gamd6d), resid = resid_d, flow = downstream$log_qsdy,
                    WTmwk = downstream$WTmwk, inund_fac = downstream$inund_fac2)
...
## Yolo ---------------------------------------------------------------------
resid_y = residuals(gamyo6d)
res_y <- data.frame(fitted = fitted(gamyo6d), resid = resid_y, flow = yolo$log_qsdy,
                    WTmwk = yolo$WTmwk, inund_fac = yolo$inund_fac2)
...
## Upstream --------------------------------------------------------
resid_u = residuals(gamu6d)
res_u <- data.frame(fitted = fitted(gamu6d), resid = resid_u, flow = upstream$log_qsdy,
                    WTmwk = upstream$WTmwk, inund_fac = upstream$inund_fac2)
### RE
...
png(filename = "./figures/validation/validation_gamd6d.png", width = 7.5, height = 9, units = "in", res = 300)
patched_d
dev.off()

png(filename = "./figures/validation/validation_gamyo6d.png", width = 7.5, height = 9, units = "in", res = 300)
patched_y
dev.off()

png(filename = "./figures/validation/validation_gamu6d.png", width = 7.5, height = 9, units = "in", res = 300)
patched_u
dev.off()
