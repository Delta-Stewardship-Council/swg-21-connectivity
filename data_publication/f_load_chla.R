...
f_load_chla <- function() {
  ...
  ybfmp_id <-contentid::store("data_publication/clean_ybfmp.csv")
  ybfmp_file <- contentid::resolve(ybfmp_id)
  wq_id <-contentid::store("data_publication/clean_discretewq.csv")
 ...
 ybfmp <- readr::read_csv(ybfmp_file)
  ...
  # export data ----------------------------------------
  print("Writing to 'data_publication/model_chla.csv'")
  readr::write_csv(join, "data_model/model_chla.csv")
  }
