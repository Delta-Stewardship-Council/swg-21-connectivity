...
## Full dataset
```{r}
data_long <- pivot_longer(chla_nuts, cols = c(din, diss_orthophos, diss_silica, inund_days,total_inund_last_year:precip_mm), names_to = "Parameter", values_to = "Value")
...
# boxplot by month
ggplot(data_long) + geom_boxplot(aes(x = factor(month), y = Value, fill = Parameter)) + facet_wrap(Parameter~., scales = "free_y") + theme_bw()
...
```{r}
covars <- chla_nuts %>%
  select(c(date, month, water_year, region, precip_mm, Q_sday, WTmwk, WTrangemwk, Sradmwk, inund_days, days_since_last_inundation, total_inund_last_year, chlorophyll, diurnal_range))

covars_corr <- covars %>%
  na.omit() %>%
  dplyr::select(precip_mm, Q_sday, WTmwk, WTrangemwk, Sradmwk, diurnal_range, inund_days, days_since_last_inundation, total_inund_last_year, chlorophyll) %>%
