...
# Flow and inundation
gam1 <- gam(log_chla~ s(Q_sday, k = 6) + s(inund_days, k = 12) , method = "REML", data = gam_data)
...
gam.check(gam2)
summary(gam2)
plot(gam2, pages = 1, all.terms = TRUE, se = TRUE, ylab = "Chlorophyll (ug/L)")
...
plot(gam1, pages = 2, all.terms = TRUE, se = TRUE, ylab = "Chlorophyll (ug/L)")
dev.off()
png("figures/gamplotresults_flow_inund.png", width = 5.5, height = 3, units = "in", pointsize = 12, res = 300)
plot(gam1, pages = 1, all.terms = TRUE, se = TRUE, ylab = "Chlorophyll (ug/L)")
dev.off()
```
