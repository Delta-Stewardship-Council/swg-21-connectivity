...
```{r}
  ...
  plot(x.idys$dowy, resid, xlab = "day of water year",   ylab = "Residuals")
  plot(x.idys$inundation, resid, xlab = "inundation flag",   ylab = "Residuals")
  par(op)
  dev.off()
  ...
  #plot(x.idys$dowy, resid, xlab = "day of water year",   ylab = "Residuals")
  plot(x.idys$inund_fac, resid, xlab = "inundation factor",   ylab = "Residuals")
  plot(x.idys$WTmwk, resid, xlab = "inundation factor",   ylab = "Residuals")
  par(op)
  dev.off()
...
med.dowy = quantile(x.idys$dowy, probs = 0.9)
...
### visualize the results for Model gam1.2 across "day of water year"
...
#~ Generate a data frame with the sequence from the minimum to the maximum
# observed value of the predictor variable: day of water year
lqsdy.15 = quantile(x.idys$log_qsdy, probs = 0.1)
lqsdy.85 = quantile(x.idys$log_qsdy, probs = 0.9)
range.dowy = range(x.idys$dowy)
dowy.ser = seq(from = range.dowy[1], to = range.dowy[2], by = 1)
n = length(dowy.ser)
new_dat_1 = data.frame(cbind(dowy = dowy.ser, log_qsdy = sample(c(lqsdy.15, lqsdy.85), n, replace = TRUE),
                             inundation = sample(c(0, 1), n, replace = TRUE)))
...
gam.i0 = subset(new_dat_1, new_dat_1$log_qsdy == lqsdy.15 & inundation == 0)
gam.i1 = subset(new_dat_1, new_dat_1$log_qsdy == lqsdy.85 & inundation == 0)
gam.i2 = subset(new_dat_1, new_dat_1$log_qsdy == lqsdy.15 & inundation == 1)
gam.i3 = subset(new_dat_1, new_dat_1$log_qsdy == lqsdy.85 & inundation == 1)

# separate actual data by inundation flag
act.i0 = subset(x.idys, x.idys$inundation == 0)
act.i1 = subset(x.idys, x.idys$inundation == 1)
...
colors = c("green", "blue")
plot(x.idys$log_chla ~ x.idys$dowy, pch = 20, col = colors[x.idys$inundation], las = 1, ylab = "", xlab = "")
#mtext(text = "Log(Chlorophyll-a)", side = 4, line = 3)
mtext(text = expression(log[e]~"(Chlorophyll-a)"), side = 2, line = 2.5)
#y_labs = c(0.0001,0.0003,0.001, 0.003,0.01,0.033,0.1,0.33, 1, 3, 8, 20, 55, 150)[c(T,F)]
#axis(side = 4, at = log(y_labs), labels = y_labs, las=1)
...
# Only for Yolo Bypass Region

## Filter to Yolo Bypass

```{r}
yolo <- alldata %>% filter(region == "yolo")

yolo$inundation = as.factor(yolo$inundation)
yolo$station    = as.factor(yolo$station)
yolo$inund_fac  = as.factor(yolo$inund_fac)
...
gam1.3 <- gam(log_chla ~ te(log_idys, dowy, by = inundation), method = "REML", data = x.idys, family = "gaussian")
gam1.4 <- gam(log_chla ~ te(log_qsdy, dowy, by = inund_fac),  method = "REML", data = x.idys, family = "gaussian")
gam1.5 <- gam(log_chla ~ te(log_qsdy, log_idys, dowy),  method = "REML", data = x.idys, family = "gaussian")
gam1.6 <- gam(log_chla ~ te(log_qsdy, dowy), method = "REML", data = x.idys, family = "gaussian")

AIC(gam0, gam1.0, gam1.1, gam1.2, gam1.3, gam1.4, gam1.5, gam1.6)
...
```{r}
  ...
  plot(x.idys$dowy, resid, xlab = "day of water year",   ylab = "Residuals")
  plot(x.idys$inund_fac, resid, xlab = "inundation factor",   ylab = "Residuals")
  #plot(x.idys$WTmwk, resid, xlab = "water temperature",   ylab = "Residuals")
  par(op)
  dev.off()


  # validate model gam1.6 - log_chla ~ te(log_qsdy, dowy, by = inundation)
  # -----------------------------------------------------------------------
  # validate the final model by plotting residuals and save to a tiff
  tiff(filename = "figures/gam_model_validation_qsdy_noi_yolo.tiff", width = 6, height = 9, units = "in", res = 100)
  resid = residuals(gam1.6)
  op = par(mfrow = c(3, 2), mar = c(5, 4, 1, 2), cex = 1.2)
  # Plot 1: Residuals vs. Fitted values; should be centered around 0
  plot(fitted(gam1.6), resid, xlab = "Fitted values", ylab = "Residuals")
  # Plot 2: histogram of the residuals; should be centered around 0
  hist(resid, xlab = 'Residuals', main = "")
  # Plot 3: is there autocorrelation in the residuals?
  acf(resid)
...
# observed value of the predictor variable: day of water year
mod_q_range = range(na.omit(x.idys$log_qsdy))
lqsdy.05 = quantile(x.idys$log_qsdy, probs = 0.05)
lqsdy.95 = quantile(x.idys$log_qsdy, probs = 0.95)
#med.dowy = median(x.idys$dowy)
med.dowy = quantile(x.idys$dowy, probs = 0.5)
#qsdy.ser = seq(from = mod_q_range[1], to = mod_q_range[2], by = 0.05)
...
# plot the actual data points separately for inundated and non-inundated
colors = c("green", "blue")
plot(x.idys$log_chla ~ x.idys$log_qsdy, pch = 20, col = colors[x.idys$inundation], las = 1, ylab = "", xlab = "")
#mtext(text = "Log(Chlorophyll-a)", side = 4, line = 3)
mtext(text = expression(log[e]~"(Chlorophyll-a)"), side = 2, line = 2.5)
#y_labs = c(0.0001,0.0003,0.001, 0.003,0.01,0.033,0.1,0.33, 1, 3, 8, 20, 55, 150)[c(T,F)]
#axis(side = 4, at = log(y_labs), labels = y_labs, las=1)
...
### visualize the results for Model gam1.6 across flow
...
#~ Use the *predict()* function to estimate model fit and standard error
preds = predict(gam1.6, newdata = new_dat_1, se.fit = TRUE)
...
# Use the polygon function to add the CI
polygon(x = c(new_dat_1$log_qsdy, max(new_dat_1$log_qsdy), rev(new_dat_1$log_qsdy), new_dat_1$log_qsdy[1]),
        y = c(new_dat_1$lower, new_dat_1$upper[length(new_dat_1$upper)], rev(new_dat_1$upper), new_dat_1$lower[1]), border = NA, 
        col = rgb(20, 200, 20, alpha = 150, maxColorValue = 255))

# Use the lines() function to add the mean model prediction
lines(x = new_dat_1$log_qsdy, y = new_dat_1$fit, lwd = 2, col = rgb(20, 150, 20, alpha = 255, maxColorValue = 255))
...
### visualize the results for Model gam1.6 across dowy
...
#~ Generate a data frame with the sequence from the minimum to the maximum
# observed value of the predictor variable: day of water year
lqsdy.15 = quantile(x.idys$log_qsdy, probs = 0.15)
lqsdy.85 = quantile(x.idys$log_qsdy, probs = 0.85)
range.dowy = range(x.idys$dowy)
#med.dowy = quantile(x.idys$dowy, probs = 0.5)
#qsdy.ser = seq(from = mod_q_range[1], to = mod_q_range[2], by = 0.05)
#qsdy.ser = seq(from = lqsdy.05, to = lqsdy.95, by = 0.02)
...
# separate by inundation flag
gam.i0 = subset(new_dat_1, new_dat_1$log_qsdy == lqsdy.15)
gam.i1 = subset(new_dat_1, new_dat_1$log_qsdy == lqsdy.85)
...
# plot(x.idys$log_qsdy ~ x.idys$inundation, pch = 20, las = 1, ylab = "", xlab = "")
# mtext(text = expression(log[e]~"(Chlorophyll-a)"), side = 2, line = 2.5)
# mtext(text = "Inundated?", side = 1, line = 2.25)
...
# Only for Cache Slough and Yolo Region (only cache does not have enough data)

## Filter to Cache Slough + Yolo

```{r}
yolo <- alldata %>% filter(region == "cache" | region == "yolo")
...
### visualize the results for Model gam1.6 across "day of water year"
...
# Only for upstream region without inundation related factors

## Filter to upstream

```{r}
sacr <- alldata %>% filter(region == "above")

sacr$inundation = as.factor(sacr$inundation)
sacr$station    = as.factor(sacr$station)
sacr$inund_fac  = as.factor(sacr$inund_fac)
...
gam0 <- gam(log_chla ~ 1, method = "REML", data = x.idys, family = "gaussian")
gam1.0 <- gam(log_chla ~ s(log_qsdy), method = "REML", data = x.idys, family = "gaussian")
gam1.1 <- gam(log_chla ~ s(log_qsdy) + s(dowy, bs = "cc"), method = "REML", data = x.idys, family = "gaussian")
gam1.2 <- gam(log_chla ~ s(WTmwk), method = "REML", data = x.idys, family = "gaussian")
gam1.3 <- gam(log_chla ~ s(WTmwk) + s(dowy, bs = "cc"), method = "REML", data = x.idys, family = "gaussian")
gam1.4 <- gam(log_chla ~ s(log_qsdy) + s(WTmwk), method = "REML", data = x.idys, family = "gaussian")
gam1.5 <- gam(log_chla ~ s(log_qsdy) + s(WTmwk) + s(dowy, bs = "cc"), method = "REML", data = x.idys, family = "gaussian")
gam1.6 <- gam(log_chla ~ te(log_qsdy, dowy), method = "REML", data = x.idys, family = "gaussian")
gam1.7 <- gam(log_chla ~ te(log_qsdy, WTmwk, dowy), method = "REML", data = x.idys, family = "gaussian")

AIC(gam0, gam1.0, gam1.1, gam1.2, gam1.3, gam1.4, gam1.5, gam1.6, gam1.7)
...
# Visualizing gam1.7 (flow + water temp. + dowy) for Sac River across day of water year
...
dowy.range = range(x.idys$dowy)
dowy.ser = seq(from = dowy.range[1], to = dowy.range[2], by = 1)
n = length(dowy.ser)

# take 10th and 90th percentile values of all other predictors
lqsdy.1 = quantile(x.idys$log_qsdy, probs = 0.1)
lqsdy.9 = quantile(x.idys$log_qsdy, probs = 0.9)
lqsdy.5 = quantile(x.idys$log_qsdy, probs = 0.5)

wtemp.1 = quantile(x.idys$WTmwk, probs = 0.1)
wtemp.9 = quantile(x.idys$WTmwk, probs = 0.9)
wtemp.5 = quantile(x.idys$WTmwk, probs = 0.5)
...
#~ Use the *predict()* function to estimate model fit and standard error
preds.1 = predict(gam1.7, newdata = new_dat_1, se.fit = TRUE)
preds.2 = predict(gam1.7, newdata = new_dat_2, se.fit = TRUE)
...
# plot the actual data points separately for inundated and non-inundated
plot(x.idys$log_chla ~ x.idys$dowy, pch = 20, col = "green", las = 1, ylab = "", xlab = "")
#mtext(text = "Log(Chlorophyll-a)", side = 4, line = 3)
mtext(text = expression(log[e]~"(Chlorophyll-a)"), side = 2, line = 2.5)
#y_labs = c(0.0001,0.0003,0.001, 0.003,0.01,0.033,0.1,0.33, 1, 3, 8, 20, 55, 150)[c(T,F)]
#axis(side = 4, at = log(y_labs), labels = y_labs, las=1)
...
# plot the actual data points separately for inundated and non-inundated
plot(x.idys$log_chla ~ x.idys$dowy, pch = 20, col = "gray", las = 1, ylab = "", xlab = "")
#mtext(text = "Log(Chlorophyll-a)", side = 4, line = 3)
mtext(text = expression(log[e]~"(Chlorophyll-a)"), side = 2, line = 2.5)
#y_labs = c(0.0001,0.0003,0.001, 0.003,0.01,0.033,0.1,0.33, 1, 3, 8, 20, 55, 150)[c(T,F)]
#axis(side = 4, at = log(y_labs), labels = y_labs, las=1)
...
visreg2d(gam1.7, xvar='log_qsdy', yvar='WTmwk', scale='response', plot.type="image")
visreg2d(gam1.7, xvar='log_qsdy', yvar='WTmwk', scale='response', plot.type="persp")
