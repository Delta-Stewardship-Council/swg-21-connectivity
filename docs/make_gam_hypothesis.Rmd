...
cat trying to work on lists
```{r}
test <- d.idys %>%
group_by(inund_fac2)%>%
 mutate(minQ = quantile(log_qsdy, 0.05),
...
d <- list(NA, NA, NA)
d[[1]] <- test %>% filter(inund_fac2 == "none")
d[[2]] <- test %>% filter(inund_fac2 == "short")
d[[3]] <- test %>% filter(inund_fac2 == "long")
library(purrr)
View(d[[3]])

predict_list <- map(d, ~ .x, predict_gam(gamd6, values = list(log_qsdy = seq(minQ, maxQ, by = 0.05))))

predict_list <- map(d, ~ .x, minQ + 2)
```
...
# Prediction datasets - Downstream
pred_gamd6_none <- predict_gam(gamd6, values = list(log_qsdy = seq(-1.47, 0.944, by = 0.05),
                                                    WTmwk = seq(-1.53, 1.87, by = 0.1),
                                                    inund_fac2 = "none"))
pred_gamd6_short <- predict_gam(gamd6, values = list(log_qsdy = seq(-0.652, 2.22, by = 0.05),
                                                    WTmwk = seq(-1.18, 0.890, by = 0.1),
                                                    inund_fac2 = "short"))
pred_gamd6_long <- predict_gam(gamd6, values = list(log_qsdy = seq(1.22, 2.23, by = 0.05),
                                                    WTmwk = seq(-0.581, 1.03, by = 0.1),
                                                    inund_fac2 = "long"))
...
# Other predictions
pred_gamd6_q <- predict_gam(gamd6, values = list(log_qsdy = c(quantile(d.idys$log_qsdy, 0.05),
                                                          quantile(d.idys$log_qsdy, 0.95), 
                                                          median(d.idys$log_qsdy)),
                                             WTmwk = c(quantile(d.idys$WTmwk, 0.05),
                                                       quantile(d.idys$WTmwk, 0.95))))
    mutate(WT_orig = (WTmwk*sd(downstream$WTmwk)+mean(downstream$WTmwk)))
  #mutate(across(.cols = everything(), round, digits = 3))

      
pred_gamd6_q2 <- predict_gam(gamd6, values = list(log_qsdy = c(quantile(d.idys$log_qsdy, 0.05),
                                                          quantile(d.idys$log_qsdy, 0.95), 
                                                          median(d.idys$log_qsdy))))
      #mutate(WTmwk = as.numeric(WTmwk))
  #mutate(across(.cols = everything(), round, digits = 3))
      
      
      
pred_gamd6 <- predict_gam(gamd6) 
```

## Constrain data
- constrain by the data we have for each inund_fac level
```{r}
