...
ggplot(Yolo_inun_nuts, aes(date, diss_nitrate_nitrite)) + 
  xlim(c(as.Date('2013-01-01'),as.Date('2020-12-31'))) +
  geom_point(color=ifelse(Yolo_inun_nuts$inundation>0, 'red', 'black')) + 
  labs(x = 'Date', y = 'NO3 + NO2 (mg/L)') +
  ggtitle('Yolo diss. NO3 + NO2')

Yolo_inun_nuts$inundation <- as.character(Yolo_inun_nuts$inundation)

#Lower range of NO3 +NO2 conc. during inundated period relative to non-inundated period

ggplot(Yolo_inun_nuts, aes(inundation, diss_nitrate_nitrite))+ geom_boxplot() + scale_x_discrete(breaks = c("0", "1")) +ggtitle('Yolo')
```
#Yolo - explore inundation vs. non-inun with box blots
```{r}

Yolo_non_nuts <- subset(Yolo_inun_nuts, inundation==0)

ggplot(Yolo_non_nuts, aes(month, diss_nitrate_nitrite))+ geom_boxplot() + 
scale_x_discrete(limits = month.abb) +
ggtitle('Yolo non-inundated period')

Yolo_flood_nuts <- subset(Yolo_inun_nuts, inundation==1)

ggplot(Yolo_flood_nuts, aes(month, diss_nitrate_nitrite))+ geom_boxplot() + 
scale_x_discrete(limits = month.abb) +
ggtitle('Yolo inundated period')
```
