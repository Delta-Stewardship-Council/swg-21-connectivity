...
source("scripts/functions/f_make_bayes_mod12dowy_dataset.R")
mod_df <- f_make_bayes_mod12dowy_dataset()
...
# Run model
jm <- jags.model("bayes_models/mod12_redlags_dowy/sam_model.JAGS",
                 data = datlist,
                 inits = initslist,
                 n.chains = 3)
...
#load("bayes_models/mod12_redlags_dowy/run_20220621.rda")
...
save(saved.state, file = "bayes_models/mod12_redlags_dowy/inits/sstate_20220621.Rda")
...
coda_sum %>%
  ...
  geom_hline(yintercept = 0, color = "red") +
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high)) +
  scale_x_discrete(labels = c("Qant", "dowy", "inund_days")) +
  scale_y_continuous("Slope of covariates") +
  theme_bw()
ggsave(filename = "bayes_models/mod12_redlags_dowy/fig_out/slope_of_betas_qant_20220621.png",
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod12_redlags_dowy/fig_out/weights_of_qant_20220621.png",
       dpi=300, width = 10, height = 8)


# save model
save(jm_coda, coda_sum, file = "bayes_models/mod12_redlags_dowy/run_20220621.rda")

# save model summary
sink("bayes_models/mod12_redlags_dowy/fig_out/jm_coda_summary.txt")
summary(jm_coda)
sink()
...
ggsave(filename = "bayes_models/mod12_redlags_dowy/fig_out/pred_vs_obs_20220621.png",
       dpi=300, width = 10, height = 8)
...
ggsave(filename = "bayes_models/mod12_redlags_dowy/fig_out/pred_obs_time_20220621.png",
       dpi=300, width = 10, height = 8)
