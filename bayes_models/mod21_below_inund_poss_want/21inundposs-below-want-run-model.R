...
datlist <- list(chl = log(chla_1$chlorophyll),
                ...
                pB = c(14, 21, 21),
                # mean of 1 day, mean of 3 days before that, etc
                nlagB = 3, # index for for loop
                alphaB = rep(1, 3)) # for prior) # for prior
...
inits_2 <- function(){
  list(sig.eps = runif(1, 0, 1),
       tau = runif(1, 0, 1),
       B = rnorm(3, 0, 1)) # for 6 B parameters, adjust as needed
}
...
# Run model
jm <- jags.model("bayes_models/mod21_below_inund_poss_want/sam_model.JAGS",
                 data = datlist,
                 inits = initslist,
                 n.chains = 3)
...
jm_coda <- coda.samples(jm,
                        variable.names = c("deviance", "Dsum", "Bstar", "wA",  "deltaA",  "wB",
                                            "deltaB",
                                           "sig", "tau"),
                        n.iter = 1000*30,
                        thin = 30) #
...
load("bayes_models/mod21_below_inund_poss_want/run_20220902.rda")
...
save(saved.state, file = "bayes_models/mod21_below_inund_poss_want/inits/sstate_20220902.Rda")
...
#   geom_hline(yintercept = 0, color = "red") +
#   geom_pointrange(aes(ymin = conf.low, ymax = conf.high)) +
#   scale_x_discrete(labels = c("0 inun[1]", "Short inun[1]", "Long inun[1]", "0 inun2[2]", "Short inun2[2]", "Long inun2[2]")) +
#   scale_y_continuous("Inun") +
#   theme_bw()
# ggsave(filename = "bayes_models/mod21_below_inund_poss_want/fig_out/slope_of_beta_inund_20220830.png",
...
coda_sum %>%
  ...
  geom_hline(yintercept = 0, color = "red") +
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high)) +
  scale_x_discrete(labels = c("Qant", "Want")) +
  scale_y_continuous("Slope of covariates") +
  theme_bw()
ggsave(filename = "bayes_models/mod21_below_inund_poss_want/fig_out/slope_of_betas_qant_20220902.png",
       dpi=300, width = 11, height = 8)
...
coda_sum %>%
  ...
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high)) +
  scale_x_discrete(labels = c("prev.2wks", "2-5wks", "5-8wks")) +
  scale_y_continuous("Weights of past Q") +
  theme_bw()
ggsave(filename = "bayes_models/mod21_below_inund_poss_want/fig_out/weights_of_qant_20220902.png",
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod21_below_inund_poss_want/fig_out/weights_of_want_20220902.png",
       dpi=300, width = 10, height = 8)

# save model
save(jm_coda, coda_sum, file = "bayes_models/mod21_below_inund_poss_want/run_20220902.rda")

# save model summary
sink("bayes_models/mod21_below_inund_poss_want/fig_out/jm_coda_summary.txt")
summary(jm_coda)
sink()
...
ggsave(filename = "bayes_models/mod21_below_inund_poss_want/fig_out/pred_vs_obs_20220902.png",
       dpi=300, width = 10, height = 8)
...
ggsave(filename = "bayes_models/mod21_below_inund_poss_want/fig_out/pred_obs_time_20220902.png",
       dpi=300, width = 10, height = 8)
