...
covars_yolo$inund_fac2 <- as.integer(ifelse(covars_yolo$inund_fac2 == "none", 1, ifelse(covars_yolo$inund_fac2 == "short", 2, 3)))

datlist <- list(chl = log(chla_yolo$chlorophyll),
                Q = c(covars_yolo$Q),
                #Srad_mwk = c(covars_yolo$Srad_mwk),
                inund_fac2 = c(covars_yolo$inund_fac2),
                inuns = 3,
...
# Run model
jm <- jags.model("bayes_models/mod17_yolo_inund_poss_wtempQant/sam_model.JAGS",
                 data = datlist,
                 inits = initslist,
                 n.chains = 3)
...
jm_coda <- coda.samples(jm,
                        variable.names = c("deviance", "Dsum", "Bstar", "Inun", "Inun2", "Inun3", "wA",
                                           "deltaA",
                                           "sig", "tau"),
                        n.iter = 1000*30,
                        thin = 30)
...
load("bayes_models/mod17_yolo_inund_poss_wtempQant/run_20220823.rda")

mcmcplot(jm_coda, col = c("red", "blue", "green"))
# Look at R-hat values. >1.02 would indicate did not converge
gelman.diag(jm_coda, multivariate = FALSE) # Looks horrible, not converging

# Save state for rerunning
# newinits <- initfind(coda = jm_coda)
# saved.state <- newinits
# save(saved.state, file = "bayes_models/mod17_yolo_inund_poss_wtempQant/inits/sstate_20220823.Rda")
...
# Inundation terms (1st 3: "Short inun", "Long inun" are intercepts offsets from 0 inundation term, 4-6: "Short inun", "Long inun" are intercepts offsets from 0 inundation term interaction with Wtemp, 7-9: "Short inun", "Long inun" are intercepts offsets from 0 inundation term interaction with Qant)
coda_sum %>%
  filter(grepl("Inun", term)) %>%
  ggplot(aes(x = term, y = estimate)) +
  geom_hline(yintercept = 0, color = "red") +
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high)) +
  scale_x_discrete(labels = c("0 inun", "Short inun", "Long inun", "Wtemp*0 inun", "Wtemp*Short inun", "Wtemp*Long inun", "Qant*0 inun", "Qant*Short inun", "Qant*Long inun")) +
  scale_y_continuous("Inun") +
  theme_bw()
ggsave(filename = "bayes_models/mod17_yolo_inund_poss_wtempQant/fig_out/slope_of_beta_inund_20220823.png",dpi=300, width = 11, height = 8)
...
coda_sum %>%
  ...
  theme_bw()
#ggsave(filename = "bayes_models/mod17_yolo_inund_poss_wtempQant/fig_out/slope_of_betas_qant_20220823.png",dpi=300, width = 11, height = 8)
...
coda_sum %>%
  ...
  theme_bw()
#ggsave(filename = "bayes_models/mod17_yolo_inund_poss_wtempQant/fig_out/weights_of_qant_20220823.png", dpi=300, width = 10, height = 8)

# save model
#save(jm_coda, coda_sum, file = "bayes_models/mod17_yolo_inund_poss_wtempQant/run_20220823.rda")

# save model summary
# sink("bayes_models/mod17_yolo_inund_poss_wtempQant/fig_out/jm_coda_summary.txt")
# summary(jm_coda)
# sink()
...
#
# ggsave(filename = "bayes_models/mod17_yolo_inund_poss_wtempQant/fig_out/pred_vs_obs_20220823.png",
...
#
# ggsave(filename = "bayes_models/mod17_yolo_inund_poss_wtempQant/fig_out/pred_obs_time_20220823.png",
#        dpi=300, width = 10, height = 8)
