...
source("scripts/functions/f_make_bayes_mod02_dataset.R")
mod_df <- f_make_bayes_mod02_dataset()
...
inits <- function(){
  list(sig.eps = runif(1, 0, 15),
       tau = runif(1, 0, 1),
       B = rnorm(8, 0, 1000)) # for 8 B parameters, adjust as needed
}
...
# Run model
jm <- jags.model("bayes_models/mod02/sam_model.JAGS",
                 data = datlist,
                 inits = initslist,
                 n.chains = 3)
...
save(saved.state, file = "bayes_models/mod02/inits/sstate.Rda")
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod02/fig_out/weights_of_qant.png",
       dpi=300, width = 10, height = 8)
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod02/fig_out/weights_of_solrad_ant.png",
       dpi=300, width = 10, height = 8)
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod02/fig_out/weights_of_airtemp.png",
       dpi=300, width = 10, height = 8)

# save model
save(jm_coda, coda_sum, file = "bayes_models/mod02/run_20211123.rda")
