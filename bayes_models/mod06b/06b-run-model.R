...
source("scripts/functions/f_make_bayes_mod06b_dataset.R")
mod_df <- f_make_bayes_mod06b_dataset()
...
# Examine only inundation = 1 data
covars <- covars %>% filter(inundation == 1)
...
png("bayes_models/mod06b/fig_out/corr_var.png",
    height = 5, width = 6, units = "in", res = 300)
pairs(covars[c(7,17,18,19)], lower.panel = panel.smooth, upper.panel = panel.cor,
