...
source("scripts/functions/f_make_bayes_mod17inundposs_wtemponly_dataset.R")
mod_df <- f_make_bayes_mod17inundposs_wtemponly_dataset()
...
# Run model
jm <- jags.model("bayes_models/mod17_yolo_inund_poss_sradonly_i/sam_model.JAGS",
                 data = datlist,
                 inits = initslist,
                 n.chains = 3)
...
load("bayes_models/mod17_yolo_inund_poss_sradonly_i/run_20220823.rda")
...
save(saved.state, file = "bayes_models/mod17_yolo_inund_poss_sradonly_i/inits/sstate_20220823.Rda")
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod17_yolo_inund_poss_sradonly_i/fig_out/slope_of_beta_inund_20220823.png",
...
coda_sum %>%
  ...
  geom_hline(yintercept = 0, color = "red") +
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high)) +
  scale_x_discrete(labels = c("Qant", "Srad")) +
  scale_y_continuous("Slope of covariates") +
  theme_bw()
ggsave(filename = "bayes_models/mod17_yolo_inund_poss_sradonly_i/fig_out/slope_of_betas_qant_20220823.png",
       dpi=300, width = 11, height = 8)
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod17_yolo_inund_poss_sradonly_i/fig_out/weights_of_qant_20220823.png",
       dpi=300, width = 10, height = 8)

# save model
save(jm_coda, coda_sum, file = "bayes_models/mod17_yolo_inund_poss_sradonly_i/run_20220823.rda")

# save model summary
sink("bayes_models/mod17_yolo_inund_poss_sradonly_i/fig_out/jm_coda_summary.txt")
summary(jm_coda)
sink()
...
ggsave(filename = "bayes_models/mod17_yolo_inund_poss_sradonly_i/fig_out/pred_vs_obs_20220823.png",
       dpi=300, width = 10, height = 8)
...
ggsave(filename = "bayes_models/mod17_yolo_inund_poss_sradonly_i/fig_out/pred_obs_time_20220823.png",
       dpi=300, width = 10, height = 8)

# TRYING to make predictions: Separate predictions by inundation factors of 0, short, long.  Run through JAGS code each time

# here

# Sample Posterior now with Qant
jm_coda2 <- coda.samples(jm,
                        variable.names = c("deviance", "Dsum", "Bstar", "Inun",  "Inun2", "Qant", "wA",
                                           "deltaA",
                                           "sig", "tau"),
                        n.iter = 1000*30,
                        thin = 30)

data_lj <- left_join(chla_yolo, covars_yolo)

coda_qant_sum <- tidyMCMC(jm_coda2, conf.int = TRUE, conf.method = "HPDinterval") %>%
  rename(pd.mean = estimate, pd.lower = conf.low, pd.upper = conf.high)

Qant_minmax <- coda_qant_sum %>% filter(grepl("Qant", term)) %>% select(pd.mean, pd.lower, pd.upper) %>% rename(minQant = pd.lower, meanQant = pd.mean, maxQant = pd.upper)

data_lj2 <- bind_cols(data_lj, Qant_minmax)

hold <- data_lj2 %>%
  group_by(as.factor(inund_fac2))%>%
  summarize(minQant_all = min(minQant),
            meanQant_all = mean(meanQant),
            maxQant_all = max(maxQant),
            minWT = quantile(Wtemp_mwk, 0.05),
            maxWT = quantile(Wtemp_mwk, 0.95),
            minSrad = quantile(Srad_mwk, 0.05),
            maxSrad = quantile(Srad_mwk, 0.95)) %>%
  ungroup()

pars <- summary(jm_coda)$statistics[,1]
m.out <- pars[1] +
  pars[2] * Qant[i] +
  pars[3] * Srad_mwk[doy1999[i]] +
  Inun[inundation[doy1999[i]]] +
  Inun2[inundation[doy1999[i]]] * Srad_mwk[doy1999[i]]
