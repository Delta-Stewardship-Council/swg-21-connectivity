...
with(out$sims.list, plot(Bstar[,3], Bstar[,4]))
with(out$sims.list, plot(Bstar[,5], Bstar[,2]))
...
coda_sum %>%
  ...
  geom_hline(yintercept = 0, color = "red") +
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high)) +
  scale_x_discrete(labels = c("Q", "Qant", "Srad_mwk", "Wtemp_mwk", "Inund_days")) +
...
# save model
save(jm_coda, coda_sum, file = "bayes_models/mod06/run_20211218.rda")
