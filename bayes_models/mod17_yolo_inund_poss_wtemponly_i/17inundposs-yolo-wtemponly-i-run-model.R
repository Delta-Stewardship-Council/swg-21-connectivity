...
# Run model
jm <- jags.model("bayes_models/mod17_yolo_inund_poss_wtemponly_i/sam_model.JAGS",
                 data = datlist,
                 inits = initslist,
                 n.chains = 3)
...
load("bayes_models/mod17_yolo_inund_poss_wtemponly_i/run_20220823.rda")
...
save(saved.state, file = "bayes_models/mod17_yolo_inund_poss_wtemponly_i/inits/sstate_20220823.Rda")
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod17_yolo_inund_poss_wtemponly_i/fig_out/slope_of_beta_inund_20220823.png",
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod17_yolo_inund_poss_wtemponly_i/fig_out/slope_of_betas_qant_20220823.png",
       dpi=300, width = 11, height = 8)
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod17_yolo_inund_poss_wtemponly_i/fig_out/weights_of_qant_20220823.png",
       dpi=300, width = 10, height = 8)

# save model
save(jm_coda, coda_sum, file = "bayes_models/mod17_yolo_inund_poss_wtemponly_i/run_20220823.rda")

# save model summary
sink("bayes_models/mod17_yolo_inund_poss_wtemponly_i/fig_out/jm_coda_summary.txt")
summary(jm_coda)
sink()
...
ggsave(filename = "bayes_models/mod17_yolo_inund_poss_wtemponly_i/fig_out/pred_vs_obs_20220823.png",
       dpi=300, width = 10, height = 8)
...
ggsave(filename = "bayes_models/mod17_yolo_inund_poss_wtemponly_i/fig_out/pred_obs_time_20220823.png",
       dpi=300, width = 10, height = 8)

# Check out if there is a relationship between Qant and Wtemp_mwk
wanted <- c("deviance", "Dsum", "Bstar", "Inun", "Inun2", "wA",
            "deltaA",
            "sig", "tau")
library(jagsUI)
# Summary of marginal posterior distributions
( out <- jags(datlist, initslist, wanted, "bayes_models/mod17_yolo_inund_poss_wtemponly_i/sam_model.JAGS", DIC=FALSE,
...
par(cex = 1.25)
with(out$sims.list, plot(Bstar[,2], Bstar[,3], xlab = "Qant", ylab = "Wtemp_mwk"))
