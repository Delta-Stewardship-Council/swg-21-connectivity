...
# Visualize Data ----------------------------------------------------------
sd(tapply(chla_yolo$chlorophyll, chla_yolo$station_wq_chl, mean))
...
chla_yolo$station_id <- as.integer(ifelse(chla_yolo$station_wq_chl == "STTD", 1, ifelse(chla_yolo$station_wq_chl == "LIS", 2, 3)))
...
# Run model
jm <- jags.model("bayes_models/mod20d_yolo_inund_poss_want_i/sam_model.JAGS",
                 data = datlist,
                 inits = initslist,
                 n.chains = 3)
...
jm_coda <- coda.samples(jm,
                        variable.names = c("deviance", "Dsum", "Bstar", "Inun", "Inun2", "wA",
                                           "deltaA", "Estar",
                                           "sig", "tau", "sig.eps", "tau.eps"), #"wB", "deltaB",
                        n.iter = 1000*30,
                        thin = 30)
...
load("bayes_models/mod20d_yolo_inund_poss_want_i/run_20220906.rda")
