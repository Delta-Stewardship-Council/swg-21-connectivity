...
# Try new lags (5; 1d, 7d, 3wks, 6wks, 9wks prior to doy) for Qant's effect on chl
...
source("scripts/functions/f_make_bayes_mod11_dataset.R")
mod_df <- f_make_bayes_mod11_dataset()

# pull out datasets
# to restrict analysis to Rio Vista, station_wq_chl = 657
chla_RV <- mod_df$chla_RV
covars_RV <- mod_df$covars_RV

# Visualize Data ----------------------------------------------------------

# check histogram of logged chlorophyll
hist(log(chla_RV$chlorophyll))

# Remove the first observation of chlorophyll because covars data don't go back that far
chla_RV_1 <- chla_RV[-1,]
# Create Model Datalist ---------------------------------------------------

datlist <- list(chl = log(chla_RV_1$chlorophyll),
                Q = c(covars_RV$Q),
                Srad_mwk = c(covars_RV$Srad_mwk),
                inund_days = c(covars_RV$inund_days),
                Wtemp_mwk = c(covars_RV$Wtemp_mwk),
                Wtemprange_mwk = c(covars_RV$Wtemprange_mwk),
                #station_id = chla_RV$station_id,
                #Nstation = length(unique(chla_RV$station_id)),
                doy1999 = chla_RV_1$doy1999,
                N = nrow(chla_RV_1),
                pA = c(1, 6, 14, 21, 21),
...
# Or load saved.state
# load("bayes_models/mod11b/inits/sstate_20220517.Rda")
# inits_2 <- function(){
...
# Run model
jm <- jags.model("bayes_models/mod11b/sam_model.JAGS",
                 data = datlist,
                 inits = initslist,
                 n.chains = 3)
...
save(saved.state, file = "bayes_models/mod11b/inits/sstate_20220517.Rda")
...
coda_sum %>%
  ...
  geom_hline(yintercept = 0, color = "red") +
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high)) +
  scale_x_discrete(labels = c("Qant", "Srad_mwk", "Wtemprange_mwk", "Wtemprange_mwk", "inund_days")) +
  scale_y_continuous("Slope of covariates") +
  theme_bw()
ggsave(filename = "bayes_models/mod11b/fig_out/slope_of_betas_qant_20220517.png",
...
coda_sum %>%
  ...
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high)) +
  scale_x_discrete(labels = c("1d ago", "2-7d ago", "prev. 1-3wk", "prev. 3-6wk", "prev. 6-9wk")) +
  scale_y_continuous("Weights of past Q") +
  theme_bw()
ggsave(filename = "bayes_models/mod11b/fig_out/weights_of_qant_20220517.png",
       dpi=300, width = 10, height = 8)


# save model
save(jm_coda, coda_sum, file = "bayes_models/mod11b/run_20220517.rda")

# save model summary
sink("bayes_models/mod11b/fig_out/jm_coda_summary.txt")
summary(jm_coda)
sink()
...
ggsave(filename = "bayes_models/mod11b/fig_out/pred_vs_obs_20220517.png",
       dpi=300, width = 10, height = 8)

# Observed vs. predicted chlorophyll as a function of time
chl_obs_dates <- as.data.frame(chla_RV_1) %>%
  select(date, doy1999)
...
ggsave(filename = "bayes_models/mod11b/fig_out/pred_obs_time_20220517.png",
       dpi=300, width = 10, height = 8)
