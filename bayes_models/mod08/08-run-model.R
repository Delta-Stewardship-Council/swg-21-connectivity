...
source("scripts/functions/f_make_bayes_mod08_dataset.R")
mod_df <- f_make_bayes_mod08_dataset()
...
png("bayes_models/mod08/fig_out/corr_var.png",
    height = 5, width = 6, units = "in", res = 300)
pairs(covars[c(7,20,21,22)], lower.panel = panel.smooth, upper.panel = panel.cor,
...
initslist <- list(inits_2(), inits_2(), inits_2())

# run this if model has been successfully run already:

# Or load saved.state
load("bayes_models/mod08/inits/sstate_2020204.Rda")
inits_2 <- function(){
  list(sig.eps = runif(1, 0, 1),
       tau = runif(1, 0, 1),
       B = rnorm(5, 0, 1)) # for 3 B parameters, adjust as needed
}
initslist <- list(list(sig.eps = saved.state[[2]][[1]]$sig.eps, tau = saved.state[[2]][[1]]$tau, B = inits_2()$B), list(sig.eps = saved.state[[2]][[2]]$sig.eps, tau = saved.state[[2]][[2]]$tau, B = inits_2()$B), inits_2())

# Run Model ---------------------------------------------------------------

# Run model
jm <- jags.model("bayes_models/mod08/sam_model.JAGS",
                 data = datlist,
                 inits = initslist,
                 n.chains = 3)
...
( out <- jags(datlist, initslist, wanted, "bayes_models/mod08/sam_model.JAGS", DIC=FALSE,
              n.chains=3, n.adapt=100, n.iter=1000, n.burnin=0) )
# Calculate cross correlations among variables from MCMC output
library(coda)
crosscorr(out$samples) # we see that the posteriors of Srad_mwk and Wtemp_RIV_mwk are no longer negatively correlated, and
...
png("bayes_models/mod08/fig_out/corr_posteriors_Srad_Wtemp.png",
    height = 5, width = 6, units = "in", res = 300)
par(cex = 1.25)
...
png("bayes_models/mod08/fig_out/corr_posteriors_Q_inund_days.png",
    height = 5, width = 6, units = "in", res = 300)
par(cex = 1.25)
...
png("bayes_models/mod08/fig_out/dens_posterior_SradWtemp.png",
    height = 5, width = 6, units = "in", res = 300)
par(cex = 1.25)
...
# Q and inund_days
png("bayes_models/mod08/fig_out/dens_posterior_Qinund.png",
    height = 5, width = 6, units = "in", res = 300)
par(cex = 1.25)
...
load("bayes_models/mod06/run_20220204.rda")
...
save(saved.state, file = "bayes_models/mod08/inits/sstate_2020204.Rda")
...
# slope of Q, and other covariates
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod08/fig_out/slope_of_betas_qant_sol_wtemp_inund_20220204.png",
       dpi=300, width = 10, height = 8)


# save model
save(jm_coda, coda_sum, file = "bayes_models/mod08/run_20220204.rda")
