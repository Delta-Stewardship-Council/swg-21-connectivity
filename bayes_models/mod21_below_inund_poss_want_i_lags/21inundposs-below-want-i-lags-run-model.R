...
datlist <- list(chl = log(chla_1$chlorophyll),
                ...
                alphaA = rep(1, 3)) # for prior) # for prior
...
# Run model
jm <- jags.model("bayes_models/mod21_below_inund_poss_want_i_lags/sam_model.JAGS",
                 data = datlist,
                 inits = initslist,
                 n.chains = 3)
...
jm_coda <- coda.samples(jm,
                        variable.names = c("deviance", "Dsum", "Bstar", "wA",  "deltaA", "Inun", "Inun2",
                                           "sig", "tau"),
                        n.iter = 1000*30,
                        thin = 30) #
...
load("bayes_models/mod21_below_inund_poss_want_i_lags/run_20220902.rda")
...
save(saved.state, file = "bayes_models/mod21_below_inund_poss_want_i_lags/inits/sstate_20220902.Rda")
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod21_below_inund_poss_want_i_lags/fig_out/slope_of_beta_inund_20220902.png",
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod21_below_inund_poss_want_i_lags/fig_out/slope_of_betas_qant_20220902.png",
       dpi=300, width = 11, height = 8)
...
coda_sum %>%
  ...
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high)) +
  scale_x_discrete(labels = c("5wks", "6wks", "7wks")) +
  scale_y_continuous("Weights of past Q") +
  theme_bw()
ggsave(filename = "bayes_models/mod21_below_inund_poss_want_i_lags/fig_out/weights_of_qant_20220902.png",
       dpi=300, width = 10, height = 8)

# save model
save(jm_coda, coda_sum, file = "bayes_models/mod21_below_inund_poss_want_i_lags/run_20220902.rda")

# save model summary
sink("bayes_models/mod21_below_inund_poss_want_i_lags/fig_out/jm_coda_summary.txt")
summary(jm_coda)
sink()
...
ggsave(filename = "bayes_models/mod21_below_inund_poss_want_i_lags/fig_out/pred_vs_obs_20220902.png",
       dpi=300, width = 10, height = 8)
...
ggsave(filename = "bayes_models/mod21_below_inund_poss_want_i_lags/fig_out/pred_obs_time_20220902.png",
       dpi=300, width = 10, height = 8)
