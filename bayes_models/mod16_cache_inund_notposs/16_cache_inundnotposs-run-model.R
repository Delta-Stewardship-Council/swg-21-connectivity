...
source("scripts/functions/f_make_bayes_mod16inundnotposs_dataset.R")
mod_df <- f_make_bayes_mod16inundnotposs_dataset()

# pull out datasets
# to restrict analysis to cache region
chla_cache <- mod_df$chla_cache
covars_cache <- mod_df$covars_cache

# Visualize Data ----------------------------------------------------------

# check histogram of logged chlorophyll
hist(log(chla_cache$chlorophyll))

# Remove the first observation of chlorophyll because covars data don't go back that far
chla_cache_1 <- chla_cache[-1,]
# Create Model Datalist ---------------------------------------------------

datlist <- list(chl = log(chla_cache_1$chlorophyll),
                Q = c(covars_cache$Q),
                Srad_mwk = c(covars_cache$Srad_mwk),
                #inund_days = c(covars_cache$inund_days),
                Wtemp_mwk = c(covars_cache$Wtemp_mwk),
                Wtemprange_mwk = c(covars_cache$Wtemprange_mwk),
                #station_id = chla_RV$station_id,
                #Nstation = length(unique(chla_RV$station_id)),
                doy2012 = chla_cache_1$doy2012,
                N = nrow(chla_cache_1),
                pA = c(14, 21, 21),
...
# Or load saved.state
load("bayes_models/mod16_cache_inund_notposs/inits/sstate_20220804.Rda")
inits_2 <- function(){
  list(
       tau = runif(1, 0, 1),
       B = rnorm(5, 0, 1)) # for 6 B parameters, adjust as needed
...
# Run model
jm <- jags.model("bayes_models/mod16_cache_inund_notposs/sam_model.JAGS",
                 data = datlist,
                 inits = initslist,
                 n.chains = 3)
...
load("bayes_models/mod16_cache_inund_notposs/run_20220804.rda")
...
save(saved.state, file = "bayes_models/mod16_cache_inund_notposs/inits/sstate_20220804.Rda")
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod16_cache_inund_notposs/fig_out/slope_of_betas_qant_20220804.png",
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod16_cache_inund_notposs/fig_out/weights_of_qant_20220804.png",
       dpi=300, width = 10, height = 8)


# save model
save(jm_coda, coda_sum, file = "bayes_models/mod16_cache_inund_notposs/run_20220804.rda")

# save model summary
sink("bayes_models/mod16_cache_inund_notposs/fig_out/jm_coda_summary.txt")
summary(jm_coda)
sink()
...
ggsave(filename = "bayes_models/mod16_cache_inund_notposs/fig_out/pred_vs_obs_20220804.png",
       dpi=300, width = 10, height = 8)

# Observed vs. predicted chlorophyll as a function of time
chl_obs_dates <- as.data.frame(chla_cache) %>%
  select(date, doy1999)
...
ggsave(filename = "bayes_models/mod16_cache_inund_notposs/fig_out/pred_obs_time_20220804.png",
       dpi=300, width = 10, height = 8)
