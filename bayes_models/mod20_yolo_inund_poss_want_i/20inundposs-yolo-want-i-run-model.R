...
datlist <- list(chl = log(chla_yolo$chlorophyll),
                ...
                N = nrow(chla_yolo),
                pA = c(1, 1, 1, 1), #c(1, 3, 3, 7, 7),
                # mean of 1 day, mean of 3 days before that, etc
                nlagA = 4, # index for for loop
                alphaA = rep(1, 4), #,
                pB = c(1, 1, 1, 1), #c(1, 3, 3, 7, 7),
                # mean of 1 day, mean of 3 days before that, etc
                nlagB = 4, # index for for loop
                alphaB = rep(1, 4)) # for prior) # for prior
...
# Run model
jm <- jags.model("bayes_models/mod20_yolo_inund_poss_want_i/sam_model.JAGS",
                 data = datlist,
                 inits = initslist,
                 n.chains = 3)
...
jm_coda <- coda.samples(jm,
                        variable.names = c("deviance", "Dsum", "Bstar", "Inun", "Inun2", "wA","wB",
                                           "deltaA","deltaB",
                                           "sig", "tau"), #
                        n.iter = 1000*30,
                        thin = 30)
...
load("bayes_models/mod20_yolo_inund_poss_want_i/run_20220830.rda")
...
save(saved.state, file = "bayes_models/mod20_yolo_inund_poss_want_i/inits/sstate_20220830.Rda")
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod20_yolo_inund_poss_want_i/fig_out/slope_of_beta_inund_20220830.png",
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod20_yolo_inund_poss_want_i/fig_out/slope_of_betas_qant_20220830.png",
       dpi=300, width = 11, height = 8)
...
coda_sum %>%
  ...
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high)) +
  scale_x_discrete(labels = c("1d", "2d", "3d", "4d")) + #("1d", "4d", "7d", "14d", "21d"))
  scale_y_continuous("Weights of past Q") +
  theme_bw()
ggsave(filename = "bayes_models/mod20_yolo_inund_poss_want_i/fig_out/weights_of_qant_20220831_1to4d.png",
...
coda_sum %>%
  ...
  geom_hline(yintercept = 1/4, color = "red") +
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high)) +
  scale_x_discrete(labels = c("1d", "2d", "3d", "4d")) + #c("1d", "4d", "7d", "14d", "21d")) +
  scale_y_continuous("Weights of past Wtmday") +
  theme_bw()
ggsave(filename = "bayes_models/mod20_yolo_inund_poss_want_i/fig_out/weights_of_want_20220831_1to4d.png",
       dpi=300, width = 10, height = 8)

# save model
save(jm_coda, coda_sum, file = "bayes_models/mod20_yolo_inund_poss_want_i/run_20220830.rda")

# save model summary
sink("bayes_models/mod20_yolo_inund_poss_want_i/fig_out/jm_coda_summary.txt")
summary(jm_coda)
sink()
...
ggsave(filename = "bayes_models/mod20_yolo_inund_poss_want_i/fig_out/pred_vs_obs_20220830.png",
       dpi=300, width = 10, height = 8)
...
ggsave(filename = "bayes_models/mod20_yolo_inund_poss_want_i/fig_out/pred_obs_time_20220830.png",
       dpi=300, width = 10, height = 8)

# Check out if there is a relationship between Qant and Wtemp_mwk
wanted <- c("deviance", "Dsum", "Bstar", "Inun", "Inun2", "wA", "wB",
            "deltaA", "deltaB",
            "sig", "tau")
library(jagsUI)
# Summary of marginal posterior distributions
( out <- jags(datlist, initslist, wanted, "bayes_models/mod20_yolo_inund_poss_want_i/sam_model.JAGS", DIC=FALSE,
...
# Plot correlation in posterior between Qant and Wtemp_mwk: nothing of note
par(cex = 1.25)
with(out$sims.list, plot(Bstar[,2], Bstar[,3], xlab = "Qant", ylab = "Want"))
