...
# Or load saved.state
load("bayes_models/mod05_Inund/inits/sstate.Rda")
inits_2 <- function(){
...
# Run model
jm <- jags.model("bayes_models/mod05_Inund/sam_model.JAGS",
                 data = datlist,
                 inits = initslist,
                 n.chains = 3)
...
load("bayes_models/mod05_Inund/run_20220307.rda")
...
save(saved.state, file = "bayes_models/mod05_Inund/inits/sstate_20220307.Rda")
...
coda_sum %>%
  ...
  geom_hline(yintercept = 0, color = "red") +
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high)) +
  scale_x_discrete(labels = c("Q", "Qant", "inund_days")) +
  scale_y_continuous("Slope of covariates") +
  theme_bw()
ggsave(filename = "bayes_models/mod05_Inund/fig_out/slope_of_betas_qant_20220307.png",
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod05_Inund/fig_out/weights_of_qant_20220307.png",
       dpi=300, width = 10, height = 8)


# save model
save(jm_coda, coda_sum, file = "bayes_models/mod05_Inund/run_20220307.rda")

# save model summary
sink("bayes_models/mod05_Inund/fig_out/jm_coda_summary.txt")
summary(jm_coda)
sink()
