...
# Run model
jm <- jags.model("bayes_models/mod04_Qant/sam_model.JAGS",
                 data = datlist,
                 inits = initslist,
                 n.chains = 3)
...
#load("bayes_models/mod04_Qant/run_20211206.rda")
...
save(saved.state, file = "bayes_models/mod04_Qant/inits/sstate.Rda")
...
coda_sum %>%
  ...
  geom_hline(yintercept = 0, color = "red") +
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high)) +
  scale_x_discrete(labels = c("Qant")) +
  scale_y_continuous("Slope of covariates") +
  theme_bw()
ggsave(filename = "bayes_models/mod04_Qant/fig_out/slope_of_betas_qant_20211216.png",
