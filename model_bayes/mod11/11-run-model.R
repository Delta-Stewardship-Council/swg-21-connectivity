...
# Or load saved.state
# load("bayes_models/mod11/inits/sstate_20220517.Rda")
# inits_2 <- function(){
...
# Run model
jm <- jags.model("bayes_models/mod11/sam_model.JAGS",
                 data = datlist,
                 inits = initslist,
                 n.chains = 3)
...
save(saved.state, file = "bayes_models/mod11/inits/sstate_20220517.Rda")
...
coda_sum %>%
  ...
  geom_hline(yintercept = 0, color = "red") +
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high)) +
  scale_x_discrete(labels = c("Qant", "Srad_mwk", "Wtemp_mwk", "Wtemprange_mwk", "inund_days")) +
  scale_y_continuous("Slope of covariates") +
  theme_bw()
ggsave(filename = "bayes_models/mod11/fig_out/slope_of_betas_qant_20220517.png",
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod11/fig_out/weights_of_qant_20220517.png",
       dpi=300, width = 10, height = 8)


# save model
save(jm_coda, coda_sum, file = "bayes_models/mod11/run_20220517.rda")

# save model summary
sink("bayes_models/mod11/fig_out/jm_coda_summary.txt")
summary(jm_coda)
sink()
...
ggsave(filename = "bayes_models/mod11/fig_out/pred_vs_obs_20220517.png",
       dpi=300, width = 10, height = 8)
...
ggsave(filename = "bayes_models/mod11/fig_out/pred_obs_time_20220517.png",
       dpi=300, width = 10, height = 8)
