...
# Run model
jm <- jags.model("bayes_models/mod17_yolo_inund_poss_Q/sam_model.JAGS",
                 data = datlist,
                 inits = initslist,
                 n.chains = 3)
...
jm_coda <- coda.samples(jm,
                        variable.names = c("deviance", "Dsum", "Bstar", "Inun", "Inun2",
                                           "sig", "tau"),
                        n.iter = 1000*30,
                        thin = 30)
...
load("bayes_models/mod17_yolo_inund_poss_Q/run_20220823.rda")
...
save(saved.state, file = "bayes_models/mod17_yolo_inund_poss_Q/inits/sstate_20220823.Rda")
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod17_yolo_inund_poss_Q/fig_out/slope_of_beta_inund_20220823.png",
...
coda_sum %>%
  ...
  geom_hline(yintercept = 0, color = "red") +
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high)) +
  scale_x_discrete(labels = c("Q", "Wtemp")) +
  scale_y_continuous("Slope of covariates") +
  theme_bw()
ggsave(filename = "bayes_models/mod17_yolo_inund_poss_Q/fig_out/slope_of_betas_qant_20220823.png",
       dpi=300, width = 11, height = 8)

# save model
save(jm_coda, coda_sum, file = "bayes_models/mod17_yolo_inund_poss_Q/run_20220823.rda")

# save model summary
sink("bayes_models/mod17_yolo_inund_poss_Q/fig_out/jm_coda_summary.txt")
summary(jm_coda)
sink()
...
ggsave(filename = "bayes_models/mod17_yolo_inund_poss_Q/fig_out/pred_vs_obs_20220823.png",
...
ggsave(filename = "bayes_models/mod17_yolo_inund_poss_Q/fig_out/pred_obs_time_20220823.png",
       dpi=300, width = 10, height = 8)
