...
source("scripts/functions/f_make_bayes_mod19inundposs_wtemponly_dataset.R")
mod_df <- f_make_bayes_mod19inundposs_wtemponly_dataset()

# pull out datasets
# to restrict analysis to above region
chla_above <- mod_df$chla_above
covars_above <- mod_df$covars_above

# Visualize Data ----------------------------------------------------------

# check histogram of logged chlorophyll
hist(log(chla_above$chlorophyll))

# Create Model Datalist ---------------------------------------------------

# Make inundation = 0 (not inundated) as category 1, and inundation = 1 (inundated) as category 2
covars_above$inundation <- as.integer(ifelse(covars_above$inundation == "0", 1, 2))

#chla_1 <- chla_above[-c(1:3),]
...
covars_above$inund_fac2 <- as.integer(ifelse(covars_above$inund_fac2 == "none", 1, ifelse(covars_above$inund_fac2 == "short", 2, 3)))

datlist <- list(chl = log(chla_above$chlorophyll),
                Q = c(covars_above$Q),
                Srad_mwk = c(covars_above$Srad_mwk),
                inund_fac2 = c(covars_above$inund_fac2),
                inuns = 3,
                #Wtemp_mwk = c(covars_above$Wtemp_mwk),
                doy1999 = chla_above$doy1999,
                N = nrow(chla_above),
                pA = c(1, 3, 3, 7, 7),
...
# Or load saved.state
#load("bayes_models/mod14_above_inund_poss_wtemp/inits/sstate_20220728.Rda")
#inits_2 <- function(){
...
# Run model
jm <- jags.model("bayes_models/mod19_above_inund_poss_sradonly/sam_model.JAGS",
                 data = datlist,
                 inits = initslist,
                 n.chains = 3)
...
load("bayes_models/mod19_above_inund_poss_sradonly/run_20220824.rda")
...
save(saved.state, file = "bayes_models/mod19_above_inund_poss_sradonly/inits/sstate_20220824.Rda")
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod19_above_inund_poss_sradonly/fig_out/slope_of_beta_inund_20220824.png",
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod19_above_inund_poss_sradonly/fig_out/slope_of_betas_qant_20220824.png",
       dpi=300, width = 11, height = 8)
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod19_above_inund_poss_sradonly/fig_out/weights_of_qant_20220824.png",
       dpi=300, width = 10, height = 8)

# save model
save(jm_coda, coda_sum, file = "bayes_models/mod19_above_inund_poss_sradonly/run_20220824.rda")

# save model summary
sink("bayes_models/mod19_above_inund_poss_sradonly/fig_out/jm_coda_summary.txt")
summary(jm_coda)
sink()
...
ggsave(filename = "bayes_models/mod19_above_inund_poss_sradonly/fig_out/pred_vs_obs_20220824.png",
       dpi=300, width = 10, height = 8)

# Observed vs. predicted chlorophyll as a function of time
chl_obs_dates <- as.data.frame(chla_above) %>%
  select(date, doy1999)
...
ggsave(filename = "bayes_models/mod19_above_inund_poss_sradonly/fig_out/pred_obs_time_20220824.png",
       dpi=300, width = 10, height = 8)
