...
source("scripts/functions/f_make_bayes_mod14inundposs_REyear_inter_dataset.R")
mod_df <- f_make_bayes_mod14inundposs_REyear_inter_dataset()
...
wy <- covars_yolo %>% select(region, date, doy1999, water_year)
chla_yolo_wy <- left_join(chla_yolo, wy)

sd(tapply(chla_yolo_wy$chlorophyll, chla_yolo_wy$water_year, mean)) #4.33

# look at chl by year
ggplot(as.data.frame(chla_yolo_wy), aes(x = as.factor(water_year), y = log(chlorophyll))) + geom_boxplot()
...
datlist <- list(chl = log(chla_yolo_wy$chlorophyll),
                Q = c(covars_yolo$Q),
                ...
                doy1999 = chla_yolo_wy$doy1999,
                N = nrow(chla_yolo_wy),
                water_year = as.numeric(as.factor(chla_yolo_wy$water_year)),
                Nwy = length(unique(chla_yolo_wy$water_year)),
                pA = c(1, 3, 3, 7, 7),
...
# Run model
jm <- jags.model("bayes_models/mod14_yolo_inund_poss_REyear_inter/sam_model2.JAGS",
                 data = datlist,
                 inits = initslist,
                 n.chains = 3)
...
jm_coda <- coda.samples(jm,
                        variable.names = c("deviance", "Dsum", "Bstar", "Estar", "Inun", "Inun2", "wA",
                                           "deltaA",
                                           "sig", "tau",
                                           "sig.eps", "tau.eps"),
                        n.iter = 1000*30,
...
load("bayes_models/mod14_yolo_inund_poss_REyear_inter/run_20220729.rda")

mcmcplot(jm_coda, col = c("red", "blue", "green"))
# Look at R-hat values. >1.02 would indicate did not converge

sink("bayes_models/mod14_yolo_inund_poss_REyear_inter/fig_out/gelman_diag.txt")
...
save(saved.state, file = "bayes_models/mod14_yolo_inund_poss_REyear_inter/inits/sstate_20220729.Rda")
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod14_yolo_inund_poss_REyear_inter/fig_out/slope_of_beta_inund_20220729.png",
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod14_yolo_inund_poss_REyear_inter/fig_out/slope_of_betas_qant_20220729.png",
       dpi=300, width = 11, height = 8)
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod14_yolo_inund_poss_REyear_inter/fig_out/weights_of_qant_20220729.png",
       dpi=300, width = 10, height = 8)

# save model
save(jm_coda, coda_sum, file = "bayes_models/mod14_yolo_inund_poss_REyear_inter/run_20220729.rda")

# save model summary
sink("bayes_models/mod14_yolo_inund_poss_REyear_inter/fig_out/jm_coda_summary.txt")
summary(jm_coda)
sink()
...
ggsave(filename = "bayes_models/mod14_yolo_inund_poss_REyear_inter/fig_out/pred_vs_obs_20220729.png",
       dpi=300, width = 10, height = 8)
...
ggsave(filename = "bayes_models/mod14_yolo_inund_poss_REyear_inter/fig_out/pred_obs_time_20220729.png",
       dpi=300, width = 10, height = 8)
