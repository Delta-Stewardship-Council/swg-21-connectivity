...
source("scripts/functions/f_make_bayes_mod14inundposs_dowy_inter_dataset.R")
mod_df <- f_make_bayes_mod14inundposs_dowy_inter_dataset()
...
# Run model
jm <- jags.model("bayes_models/mod14_yolo_inund_poss_dowy_inter/sam_model.JAGS",
                 data = datlist,
                 inits = initslist,
                 n.chains = 3)
...
load("bayes_models/mod14_yolo_inund_poss_dowy_inter/run_20220729.rda")
...
save(saved.state, file = "bayes_models/mod14_yolo_inund_poss_dowy_inter/inits/sstate_20220729.Rda")
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod14_yolo_inund_poss_dowy_inter/fig_out/slope_of_beta_inund_20220729.png",
       dpi=300, width = 11, height = 8)

# slope of Q, Qant, dowy
...
coda_sum %>%
  ...
  geom_hline(yintercept = 0, color = "red") +
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high)) +
  scale_x_discrete(labels = c("Qant","Wtemprange_mwk", "dowy")) +
  scale_y_continuous("Slope of covariates") +
  theme_bw()
ggsave(filename = "bayes_models/mod14_yolo_inund_poss_dowy_inter/fig_out/slope_of_betas_qant_20220729.png",
       dpi=300, width = 11, height = 8)
...
coda_sum %>%
  ...
  theme_bw()
ggsave(filename = "bayes_models/mod14_yolo_inund_poss_dowy_inter/fig_out/weights_of_qant_20220729.png",
       dpi=300, width = 10, height = 8)

# save model
save(jm_coda, coda_sum, file = "bayes_models/mod14_yolo_inund_poss_dowy_inter/run_20220729.rda")

# save model summary
sink("bayes_models/mod14_yolo_inund_poss_dowy_inter/fig_out/jm_coda_summary.txt")
summary(jm_coda)
sink()
...
ggsave(filename = "bayes_models/mod14_yolo_inund_poss_dowy_inter/fig_out/pred_vs_obs_20220729.png",
       dpi=300, width = 10, height = 8)
...
ggsave(filename = "bayes_models/mod14_yolo_inund_poss_dowy_inter/fig_out/pred_obs_time_20220729.png",
       dpi=300, width = 10, height = 8)
